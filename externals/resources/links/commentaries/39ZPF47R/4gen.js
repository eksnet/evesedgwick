  ...
  *******************************************************************************************/
  ...
  var DocumentManager = (function () {
      ...
      ScrollViewManager.prototype.pagePosition = function() {
        ...
        var page = this.documentManager.currentPageNum();
        var page_elm = $("#outer_page_" + page);
        var pos = ($(window).scrollTop() - page_elm.offset().top) / page_elm.height();
        return [page, pos];
      }
      ...
      DocumentManager.prototype.removeInsteadOfBlurring = function () {
        // This function only gets called when there's actually a page
        // that might get blurred.
        // That way we don't hand out unnecessary cookie assignments and
        // dilute our A/B test data with noise.
        if ((/iPhone|iPod|iPad|Android|BlackBerry/).test(navigator.userAgent)) {
          // We're on mobile.
          return true;
        }
        if (typeof Scribd === "undefined" || !Scribd.test_choice_value || typeof $ === "undefined") {
          return true;
        }
        // Scribd.test_choice_value will do cookie assignment
        // if it hasn't been done already.
        return (Scribd.test_choice_value('blur_test') !== 'test');
      }
