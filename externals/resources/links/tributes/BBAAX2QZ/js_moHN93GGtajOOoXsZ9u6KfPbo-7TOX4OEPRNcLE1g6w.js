...
tb_show = function (caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	try {
      ...
      var params = [];   // Amherst
			...
			$("#TB_window").append("<a href='' id='TB_ImageOff' title='" + settings.close + "'><img id='TB_Image' src='"+url+"' width='"+imageWidth+"' height='"+imageHeight+"' alt='"+caption.replace(/'/, "&squot;")+"'/></a>" + "<div id='TB_caption'>"+caption+"<div id='TB_secondLine'>" + TB_imageCount + TB_PrevHTML + TB_NextHTML + "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='" + settings.close + "'>" + settings.close + "</a> " + settings.esc_key + "</div>");
