...
(function( $ ){
			...
			return this.each(function(){
				...
				if ( ! data ){
					...
					var $sliderContainer = $slider.find(options.slider),
					    $sliderControls = $slider.next('.controls'),
					    $items = $sliderContainer.find( options.items ),
					    originalWidth = 1;

					$items.each(function(){
						// fixed width
						// check jquery version and load the width correctly
						if (jq_version[1] >= 8) {
							$(this).innerWidth($slider.innerWidth());
						} else {
	...
	function slideTo( e, $slider, x, i, t ){
		...
		var slideWidth = 0;
		for (var w=0;w<i;w++) {
			slideWidth+= $slider.items.filter('*:eq('+w+')').outerWidth(true);
		}
