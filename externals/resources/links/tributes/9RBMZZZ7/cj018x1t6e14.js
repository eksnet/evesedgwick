...
mB=(this.zD336&&zD336>0)&&(g==336)&&(h==280)?3:1;ax=(z&2?'':'/'+bp+((td==uy)&&c.length?'abt.'+c:td))+(z&4?'':'/'+c+'_'+gs+';')+zap+'svc='+s+';site='+(zgs.length?zgs:gs)+';t='+zTa+';bt='+zBT+';bts='+zBTr+zST(2,';st=')+';pc='+zpc+(z?';oe=iso-8859-1':'')+';auc='+(++zAUC)+';fd='+zFDT+';fs='+zFST+(this.zir?';r='+zir:'')+';sp2='+(zbXP2?1:0)+(zs>0?';go='+zGSk:'')+';a='+zc(0,'aut=')+tm+';kw='+k+';chan='+c+';syn=about;tile='+p+(mB==3?';af=0':'')+';r='+zRf+(bT?';dcopt=ist':'')+';sz='+d+';u='+zOr+';dc_ref='+escape(zWl)+';ord='+(bH?'1':o)+zOr
if((g==468)||(g==728))zGSk+=4;if(bT)zDc=0;if(o=='8')return zas+'/adx'+ax
zau(g==160||g==728||((g==336)&&(h==280))?32769:1,g,h,sn,zas,ax,mB)}
