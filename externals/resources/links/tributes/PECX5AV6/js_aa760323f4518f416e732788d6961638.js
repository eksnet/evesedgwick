...
;
/*
 * This behavior is intended as a temporary measure to fix the messed up user login block on some pages.
 * This is a BAD solution.
 */
Drupal.behaviors.fixLogin = function (context) {
  
  var userLogin = $("#block-user-0");
  var logout = $("#block-menu_block-1 .menu-name-menu-tools ul li.first a");
  
  // userLogin block is broken/missing
  if( userLogin.length == 0) {
    // Check to see if we have a logout link
    if (logout.attr("href") != "/logout") {
      // Add a login link...and cry in our soup.
      $("#block-menu_block-1 .menu-name-menu-tools ul").prepend('<li class="first menu-mlid-4"><a href="/user/login?destination=' + encodeURIComponent(document.location.pathname) + '">Login</a></li>');
    }
  }
...
/*
 * This behavior enables the "find out more" text replacement jawn. If js is
 * enabled, we show the "find out more" button and bind a click action to it to
 * hide the teaser field, and show the body field, effectively replacing the 
 * text.
 */
Drupal.behaviors.introTextReplace = function(context) {
  var container = 'div.intro-text-replace';
  var button = 'div.intro-text-replace-button-wrap';
  var teaser = 'div.views-field-teaser';
  var body = 'div.views-field-body';
  if ($(container + ' ' + button + ' a').length) {
    if ($(container + ' ' + teaser).text() == $(container + ' ' + body).text()) {
      return;
    }
    $(container + ' ' + button).show();
    $(container + ' ' + button + ' a').bind('click', function() {
      if ($(container + ' ' + teaser).is(':hidden')) {
        $(container + ' ' + body).slideUp('fast', function() {
          $(container + ' ' + teaser).slideDown('fast');
        });
        $(container + ' ' + button + ' a').text($(this).attr('name'));
      }
      else {
        $(container + ' ' + teaser).slideUp('fast', function() {
          $(container + ' ' + body).slideDown('fast');
        });
        $(container + ' ' + button + ' a').attr('name', $(this).text());
        $(container + ' ' + button + ' a').text('hide');
      }
      return false;
    });
  }
...
Drupal.expandoBlock = function(container_selector, button_id) {
  var container = $(container_selector);
  if (container.length) {
    var blocktitle = $('h2.block-title', container);
    var menu = $('ul', container);
    if (menu.length && blocktitle.length) {
      var title = blocktitle.text();
      var button = '<a class="button button-up" id="'+button_id+'" href="#">' + title + '</a>';
      blocktitle.replaceWith(button);
      button = $('#'+button_id);
      button.bind('click', function() {
        menu.slideToggle('fast', function() {
          if (menu.is(':hidden')) {
            button.addClass('button-up').removeClass('button-down');
          }
          else {
            button.addClass('button-down').removeClass('button-up');
          }
        });
        return false;
      });
...
Drupal.behaviors.expandoBlocks = function(context) {
  Drupal.expandoBlock('div.pane-menu-block-3,#block-menu_block-3', 'post-content-button');
  Drupal.expandoBlock('#block-commons_core-5', 'actions-button');
  // Some additional munging for anonymous post content button
  if ($('body').hasClass('not-logged-in') && $('div.pane-menu-block-3 ul,#block-menu_block-3 ul').length) {
    $('div.pane-menu-block-3 ul li,#block-menu_block-3 ul li').hide();
    $('div.pane-menu-block-3 ul,#block-menu_block-3 ul').append('<li>All members are invited to contribute to this site. Please <a href="/user/login">log in</a> or <a href="/user/register">register</a> to get started.</li>');
  }
}

Drupal.behaviors.scholarClassLinks = function(context) {
  var block = $('#block-views-classes_nav-block_1');
  var scholars_link = $('.block-menu_block a[href*="scholars/scholars"]');

  if (block.length && scholars_link.length) {
    $('.item-list ul', block)
      .clone(true)
      .insertAfter(scholars_link)
      .addClass('class-links');
    block.remove();

    if ($('#scholars-list').length) {
      scholars_link.click(function(){
        $('.class-links', $(this).parent()).slideToggle('fast');
        return false;
      });
      if (scholars_link.hasClass('active')) {
        $('.class-links li:first-child a').addClass('active');
        scholars_link.removeClass('active');
      }
    } else {
      $('.class-links', scholars_link.parent()).hide();
    }
  }
}

Drupal.behaviors.fudgeHomepageTermNavigation = function(context) {
  if (!$('body[id^="pid-home"]').length) {
    return;
  }
  if ($('div.pane-site-specific-topics-terms-panel-pane-2 a.topics-tab-navigation').length) {
    $('a.topics-tab-navigation').each(function() {
      var href = $(this).attr('href');
      var tid = href.replace(/[^0-9]*/g, '');
      $(this).attr('href', '/home?tid=' + tid + '#topics-term-description');
...
Drupal.behaviors.termLimit = function(context) {
  $('.view-content div[class^="views-field-tid"] ul, .view-content div[class^="views-field-groups"]ul').each(function() {
    if ($(this).hasClass('processed')) {
      return;
    }
    $(this).addClass("processed");
    $('li:gt(4)', this).remove();
    $('li:last', this).addClass('last').addClass("processed");
  });
  $('.view-content div[class^="views-field-tid"] ol, .view-content div[class^="views-field-groups"] ol').each(function() {
    if ($(this).hasClass('processed')) {
      return;
    }
    $(this).addClass("processed");
    ...
    var hidden = $('li:gt(4)', this).hide();
    var fakeLast = $('li:eq(4)', this)
      .addClass('last')
      .append('<span class="more-terms"><a href="#">more</a>...</span>');
    $('.more-terms a', this).click(function(){
      $(this).parent().remove();
      fakeLast.removeClass('last');
      hidden.show();
      return false;
    });
  });
...
/* This behavior governs the tabbed navigation on for Topics. At the moment all
 * this does is applies the class "active" to the first link in the list if
 * there is not already an active link 
 */
Drupal.behaviors.tabbedNavigation = function(context) {
  var container = 'div.view-id-site_specific_topics_terms.view-display-id-panel_pane_2';
  if (!$(container).length) {
    return;
  }
  var firstlink = 'div.view-content a:first';
  var activelink = 'div.view-content a.active';
  var firstcount = $(container + ' ' + firstlink).length;
  var activecount = $(container + ' ' + activelink).length;
  if (firstcount > 0 && activecount == 0) {
    $(container + ' ' + firstlink).addClass('active');
  }
}

Drupal.behaviors.termPageNavigation = function(context) {
  if ($('.views-exposed-form div#edit-featured-wrapper').length) {
    $('.views-exposed-form div#edit-featured-wrapper').parents('div.views-exposed-widget').hide();
  }
  var container = '#block-views-b658d07af56f265108bab06984e6190f div.view-content';
  if(!$(container).length) {
    return;
  }
  $(container + ' ul:not(.active)').hide();
  if ($(container + ' li a.active').length) {
    var active_parent = $(container + ' li a.active').parents('div.item-list');
  }
  else if ($(container + ' h3 a.active').length && $(container + ' h3 a.active').parents('h3').next('ul').length) {
    var active_parent = $(container + ' h3 a.active').parents('div.item-list');
  }
  if (!$(active_parent).length) {
    return;
  }

  $(active_parent).addClass('active-wrapper');
  $(container + ' div.item-list:not(.active-wrapper)').addClass('inactive-wrapper');
  $('ul', active_parent).show();
  $('ul', active_parent).addClass('active-list-wrapper');
  $('h3', active_parent).addClass('active-parent-wrapper');
  $('h3.active-parent-wrapper a').addClass('active-parent');
  $('h3', active_parent).append('<div class="accordion-trigger"><a href="javascript:;" class="accordion-trigger-link accordion-trigger-open">-</a></div>');
  
  $(container + ' h3:not(.active-parent-wrapper)').append('<div class="accordion-trigger"><a href="javascript:;" class="accordion-trigger-link accordion-trigger-closed">+</a></div>');

  var accordion_trigger_click_open = function() {
    var wrapper = $(this).parents('div.item-list');
    if ($(container + ' ul:visible').length) {
      $(container + ' ul:visible').slideUp('fast', function() {
        $('ul', wrapper).slideDown('fast');
      });
    }
    else {
      $('ul', wrapper).slideDown('fast');
    ...
    $('.accordion-trigger-open').text('+');
    $('.accordion-trigger-open').addClass('accordion-trigger-closed');
    $('.accordion-trigger-open').removeClass('accordion-trigger-open');
    $(this).removeClass('accordion-trigger-closed');
    $(this).addClass('accordion-trigger-open');
    $(this).text('-');
    $('.accordion-trigger-open, .accordion-trigger-closed').unbind('click');
    $('.accordion-trigger-open').bind('click', accordion_trigger_click_close);
    $('.accordion-trigger-closed').bind('click', accordion_trigger_click_open);
    return false;
  }

  var accordion_trigger_click_close = function() {
    var wrapper = $(this).parents('div.item-list');
    $('ul', wrapper).slideUp('fast');
    $(this).removeClass('accordion-trigger-open');
    $(this).addClass('accordion-trigger-closed');
    $(this).text('+');
    $('.accordion-trigger-open, .accordion-trigger-closed').unbind('click');
    $('.accordion-trigger-open').bind('click', accordion_trigger_click_close);
    $('.accordion-trigger-closed').bind('click', accordion_trigger_click_open);
...
Drupal.behaviors.flickrMunger = function(context) {
  if (!$('body.page-node div.field-field-flickr-photo div.flickr-photo-img').length) {
    return;
  }
  $('div.field-field-flickr-photo div.field-item div.flickr-photo-img').hide();
  $('div.field-field-flickr-photo div.field-item div.flickr-photo-img:first').show();

}

// We expose the edit link for group types in the "actions" block, so hide it in
// the tabs.
Drupal.behaviors.hideEditOnGroup = function(context) {
  var body = $('body')[0];
  if ($(body).hasClass('og-context')) {
    for (var delta in body.classList) {
      var match = body.classList[delta].match('og-context-(\\d+)');
      if (match) {
        var group_nid = match[1];
        $('#content-tabs-inner a[href="/node/'+group_nid+'/edit"]').hide();
        break;
      }
    }
  }
...
Drupal.behaviors.fixHeights = function(context) {
  ...
  if ($('.front #column-top-left .homepage-section .view-content').length) {
    $('.front #column-top-left .homepage-section:first .view-content').equalHeights();
    $('.front #column-top-left .homepage-section:last .view-content').equalHeights();

  }
}

Drupal.behaviors.externalLinks = function(context) {
  var selectors = '.map-link a';
  $(selectors).attr('target', '_blank');
  $(selectors).attr('rel', 'external');
}
