/*
$Id: wtbase.js 118735 2013-01-02 23:12:04Z scott.wolynski $ 
 */

// intialize vars. Should be initialized in wtinit.js
var DCS = DCS || {};
var WT = WT || {};
var DCSext = DCSext || {};
var wtbaseInitialized = false;
...
function dcsCQP(){
    ...
    if ( (qry && (qry.indexOf("wt.mc_id")!==-1) && (qry.indexOf("wt.mc_ev")===-1)) || ((WT.mc_id!==null) && (WT.mc_id!=="") && (WT.mc_id!==undefined)) ) {
        return "click";
    }
    else{
        return "";
	...
	if (cmatchCount>0){
		...
		if ((cmatchCount>1)&&(name==dcsInit.fpc)){
			...
			for (i=0;i<cmatchCount;i++){
				var lv=parseInt(dcsGetCrumb(cmatch[i],"lv"), 10);
...
function dcsFPC(offset){
	...
	if (document.cookie.indexOf(name+"=")==-1){
		...
		var lv=parseInt(dcsGetCrumb(c,"lv"),10);
		var ss=parseInt(dcsGetCrumb(c,"ss"),10);
...
function dcsMultiTrack(){
	// if dcsInit wasn't initialized on first load, init now
	if (!wtbaseInitialized) {
		wtbaseInit();
	}
...
function dcsMeta(){
	...
	if (document.all){
	   try {
		elems=document.all.tags("meta"); // removed parent, can not make calls cross subdomain
	   } catch (e) {}
	}
	else if (document.documentElement){
	   try {
		elems=document.getElementsByTagName("meta"); // removed parent, can not make calls cross subdomain
	   } catch (e) {}
	}
	if (typeof(elems)!="undefined"){
...
function wtbaseInit() {
	if ((typeof dcsInit !== 'undefined') && dcsInit.enabled){
		dcsVar();
		dcsMeta();
		WT.mc_ev=dcsCQP();
		dcsFunc("dcsAdv");
		...
		//dcsTag();
		wtbaseInitialized = true;
	}
}

//  add required meta tags from cookie values for WT (NYT-S cookie, nyt-d cookie, RMID cookie)
(function(){
    ...
    // write meta tags for tracking, these are the required tags for WT
    function addPageTags() {
        addMetaTag("WT.cg_n", "Video");                  // content group name
        addMetaTag("WT.z_rcgn", "Video");                // content group name
