/* m-1044~/static/email-subscription/js/email-subscription.js */
(function() {
    "use strict";
    ...
    templates = {
        ...
        'australian-politics': {
            ...
            'copy': "All the latest developments and analysis from the campaign trail, delivered to you every weekday.",
...
/* m-1044~/static/outbrain/main_v2.js */
jQ(function(){
...
/* m-2041~static/js/yappy7.js */
require(['http://id.guim.co.uk/static/cs/js/guardian.identity.clientside-toolkit.js'], function(toolkit){

    jQ(function(){
        var user = toolkit.localUserData();
        if(user)
        {
            var user_id = user.id;
            var display_name = user.publicFields.displayName;
            var jsonUrl = yappy_url_root+'/message/for/'+user.id
            jQ.getJSON(jsonUrl, function(data) {
                var msg = data.message;
                var msg_id = data.message_id
                if(msg != 'No message for user'){
                    msg = display_name+', '+msg;
                    jQ("#yappy-message-text").html(msg);
                    jQ("#yappy-message").show();
                    jQ("#yappy-message").css("top", (jQ(document).scrollTop() +180));
                    var promoLeft = jQ("#crumb1").offset().left;
                    var currentLeft = jQ("#yappy-message").offset().left;
                    var toMove = (currentLeft - promoLeft)-5;
                    var newPosition = "+="+toMove+"px";
                    jQ("#yappy-message").animate({"right": newPosition}, "slow");
                    jQ("#yappy-message-close").click(function() {
                        jQ("#yappy-message").hide();
                        var ackUrl = yappy_url_root+'/acknowledge/message/'+msg_id
                        jQ.getJSON(ackUrl, function(data) {
