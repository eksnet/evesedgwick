...
TAGX.EventProxy = (function () {
    ...
    return {
        ...
        trigger: function (eventName, eventData) {
            ...
            if (eventDataCopy && eventDataCopy.action !== "impression") {
                datum.assetUrl = TAGX.Utils.getCanonicalUrl();
                datum.url = location.href;
                datum.referrer = document.referrer;
                datum.subject = "module-interactions";
                datum.moduleData = TAGX.Utils.stringifyJson(eventDataCopy);
                etEvtName = "module-interaction";  
                eventObject.trigger(etEvtName, datum); // for ET only
            } else {
                eventObject.trigger(etEvtName, eventDataCopy); // for ET only  
            }
        }
    };
})();
...
(function (root, factory) {
   ...
   var
        ...
        assetUrl = TAGX.Utils.getCanonicalUrl(),
        sourceApp = getMetaTag('sourceApp');

   // URL encode.
   src = host + '/?url=' + encodeURIComponent(location.href) + '&assetUrl=' + encodeURIComponent(assetUrl.replace(/\.(stg|dev)\./, ".")) + '&referrer=' + encodeURIComponent(document.referrer) + '&CG=' + encodeURIComponent(getMetaTag('CG')) + '&SCG=' + encodeURIComponent(getMetaTag('SCG'));
   // if sourceApp is available use it
   if(sourceApp && sourceApp != "") {
      src += '&sourceApp=' + encodeURIComponent(sourceApp);     
   }
